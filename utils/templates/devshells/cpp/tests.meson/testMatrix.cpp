...
TYPED_TEST(MatrixTest, MatrixGetter) {
  EXPECT_EQ(this->matrix3x3.getRows(), std::size_t{3});
  EXPECT_EQ(this->matrix3x3.getCols(), std::size_t{3});
...
TYPED_TEST(MatrixTest, OutOfRangeThrowing) {
  ...
  EXPECT_THROW(this->matrix3x3.set(3, 3, -1), std::out_of_range);
  EXPECT_THROW(this->matrix3x3.set(-1, 0, -1), std::out_of_range);
  EXPECT_THROW(this->matrix3x3.set(0, -1, -1), std::out_of_range);
}

TYPED_TEST(MatrixTest, ConstructorThrowsOnZeroDimensions) {
  EXPECT_THROW({ Matrix<TypeParam> mat(0, 3); }, std::invalid_argument);
  EXPECT_THROW({ Matrix<TypeParam> mat(3, 0); }, std::invalid_argument);
  EXPECT_THROW({ Matrix<TypeParam> mat(0, 0); }, std::invalid_argument);
}
